...
def xor(str, char):
	return ''.join(chr(ord(a) ^ ord(b)) for a,b in zip(str,char*len(str)))
...
def maxMessage(str):
  ascii=binascii.a2b_hex(str).decode()  
  scores={}
  for x in range(256):
    scores[x]=score(xor(ascii,chr(x)))
  maxx=max(scores, key=scores.get)
  return (xor(ascii,chr(maxx)))

print(maxMessage(input()))